 ...
 *
 *  The operations are parallelized on samples.
 *
 ...
 */
    ...
    def tracksSaved(nameOf: Sample => String): SampleFile = 
      SampleFile.inMemorySampleFile {
        ParGMQL.runVec { 
          samples.toVector.map(s => () => s.trackSavedAs(nameOf(s)))
        }
      } 
...
/** Here is the implementation of the operations above.
 */
  ...
  object SFOps {
    ...
    def onRegion(samples: SampleFile)(f: BedFile => BedFile): SampleFile = 
      SampleFile.inMemorySampleFile {
        ParGMQL.runVec {
          for (s <- samples.toVector) 
          yield () => s.bedFileUpdated(f(s.bedFile).serialized) 
       }
     }
    ...
    def projectS
      (samples: SampleFile)
      (onSample: (String, Sample => Any)*)
    : SampleFile = SampleFile.inMemorySampleFile {
        ParGMQL.runVec {
          for (s <- samples.toVector) 
          yield () => {
            val ps = onSample map { kf => val (k, f) = kf; (k, f(s)) }
            val ns = s.mErased ++ (ps: _*)
            ns.trackSerialized
          }
        }
     }
    ...
    : SampleFile = SampleFile.transientSampleFile {
        ParGMQL.run {
          val theseSamples = samples.serialized.toVector
          val otherSamples = that.serialized.toVector
          for (
            s <- theseSamples;
            ts = otherSamples.filter(t => joinby(s, t));
            if !ts.isEmpty;
            bs = ts.map(_.bedFile).toSeq) 
          yield () => {
            val nb  = s.bedFile.differenceR(bs: _*)(exact)
            s.bedFileUpdated(nb.serialized)
          }
        }
    }
    ...
    : SampleFile = SampleFile.transientSampleFile {
        ParGMQL.runBig {
          ...
          yield () => {
            val ns = onSample(s, t)
            val nb = onRegion(s.bedFile, t.bedFile).serialized
            ns.bedFileUpdated(nb)
          }
        }
    }
    ...
    : SampleFile = groupby.length match {
      ...
      case _ => SampleFile.inMemorySampleFile {
        ParGMQL.runVec {
          import synchrony.iterators.AggrCollections.implicits._
          val l = (s: Sample) => Map(groupby.map(g => g -> s[Any](g)): _*)
          val ss = samples.groupby(l)(OpG.keep)
          for ((m, acc) <- ss.toVector) 
          yield () => {
            val b = onRegion(acc.map(_.bedFile).toSeq).serialized
            Sample(m, b)
          }
        }
      }
