...
object BothParOpsImplicit {

/** Use parallel SampleFileParOps as SampleFileOps
 *  Use parallel BedFileParOps as BedFileOps
 */

  import synchrony.pargmql.{ SampleFileParOps => SampleFileOps } 
  import SampleFileOps.SampleFile 
  import synchrony.pargmql.BedFileParOps
  import synchrony.genomeannot.GenomeAnnot
  import synchrony.genomeannot.{ GenomeAnnot, SplitFiles }


  class BFOpsWithImplicitAction[A](mm: (=> SampleFile) => A) {

    val SampleFile       = SampleFileOps.SampleFile
    val Sample           = SampleFileOps.Sample
    val BedFileOps       = BedFileParOps
    ...
    val SFOpsExplicit    = synchrony.gmql.SampleFileOps.SFOps
    ...
    type SFOpsExplicit   = SampleFileParOps.SFOps

/** SBedFile is the special file format for BedFile-level parallelism
 */
  ...
  } // BFOpsWithImplicitAction


  def iaMaterialize(samples: => SampleFile) = {
    synchrony.iterators.FileCollections.newTMPDIR()
    SampleFile.onDiskSampleFile(samples.serialized.filename)
  }


  val AlwaysMaterialize = new BFOpsWithImplicitAction(iaMaterialize _)


  def iaSortIfNeeded(samples: => SampleFile) = { 
    synchrony.iterators.FileCollections.newTMPDIR()
    SampleFile.onDiskSampleFile(
      new SampleFileOps.SFOps(samples).tracksSortedIfNeeded.serialized.filename
    )
  }

  val AlwaysSortIfNeeded = new BFOpsWithImplicitAction(iaSortIfNeeded _)


} // End object BedFileParOpsImplicit 
