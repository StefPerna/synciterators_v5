...
object GMQL {
  ...
  //
    ...
    : DB = DB.select(sample, region)(this)
    ...
    : DB = DB.project(sample, region)(this)
    

    def flatGroupBy[G, R](
      sample: GroupBySample[G] = DB.NoSampleGroup,
      region: GroupByRegion[R] = DB.NoRegionGroup)
    : DB = DB.flatGroupBy(sample,region)(this)


    def flatGroupByS[G](sample: BySample[G]): DB =
      DB.flatGroupByS(sample)(this)


    def flatGroupByR[R](region: ByRegion[R]): DB =
      DB.flatGroupByR(region)(this)
 ...
 */
    ...
    def map
      ...
      (edb: DB)
    : DB = DB.map(region, joinby)(this,edb)
    ...
    def difference
      (joinby: OnSample[CanJoinS] = DB.OnSample(),
       exact: Boolean = false)
      (edb: DB)
    : DB = DB.difference(joinby, exact)(this, edb)
    ...
    def joinNearest
      ...
      (edb: DB)
    : DB = DB.joinNearest(limit, up, down, inclMid, pred,
                          joinbyS, joinbyR,
                          outputS, outputR, orderR)(this, edb)
   ...
   *  @customization is the customization function for optional Bed fields.
   ...
   */
    ...
    : DB =  new DB(onDiskSampleFile(samplelist, customization, settings))
   ...
   */
    ...
    : DB = new DB(altOnDiskSampleFile(path)(samplelist,customization,settings))
   ...
   */
    def altOnDiskEncodeNPDB
      ...
      (samplelist: String,
       settings: SampleFileSettings = altSettingsSampleFile(path))
    : DB = altOnDiskDB(path)(samplelist, toEncodeNPSampleFile _, settings)
   ...
   */
    ...
    def sortTracks
      (db:DB)
      (implicit ordering: Ordering[Bed] = Sample.ordering)
    : DB = DB(for (s <- db.samples.eiterator) yield s.trackSorted(ordering))
   ...
   */
    ...
    def select
      ...
      (db: DB)
    : DB = {
      def it = for (s <- db.samples.eiterator; if sample.p.forall(f => f(s))) 
               yield if (region.p.isEmpty) s 
                     else s.trackUpdated {
                       for (r <- s.track; if region.p.forall(f => f(s,r)))
                       yield r
                     } 
      if (sample.p.isEmpty && region.p.isEmpty) db else DB(it) 
    }
   ...
   */
    ...
    def extend(sample: OnSample[SProj])(db: DB): DB = {
      val (sproj, sprojAggr) = split(sample.p.to(Vector), Vector(), Vector())
      val projs = sproj.map(_.toPair)
      val aggrs = mkAggrFromSProj(sprojAggr)
      def it = 
        for (s <- db.samples.eiterator)
        yield { 
          val snew = s.mOverwrittenWith(projs:_*) 
          if (sprojAggr.isEmpty) snew
          else snew.mOverwritten(s.track.flatAggregateBy(aggrs(s)))
        }
      if (sproj.isEmpty && sprojAggr.isEmpty) db else DB(it) 
    }
    ...
    : DB = DB {
      ...
      yield 
        Step0 { 
          s
          .mErased
          .mMerged(Map(projs.map { case (k, f) => k -> f(s) } :_*))
        }
       .Step1 { s0 =>
          if (sprojAggr.isEmpty) s0
          else s0.mOverwritten(s.track.flatAggregateBy(aggrs(s)))
        }
       .Step2 { s1 =>
          if (projr.isEmpty) s1
          else s1.trackUpdated {  
            for (r <- s.track)
            yield r
                  .eraseMisc()
                  .overwriteMisc {
                     Map(projr.map { case (k, f) => k -> f(s, r) } :_*)
                  }
          }
        }
       .Done
   ...
   */
    ...
    def flatGroupByS[G](sample: BySample[G])(db: DB): DB =  DB {
    ...
    def flatGroupByR[R](region: ByRegion[R])(db: DB): DB = DB {
    ...
    def flatPartitionByR[R](region: ByRegion[R])(db: DB): DB = DB {
 ...
 */
   ...
   */
    ...
    : DB = { 
     ...
     .Step1 { db =>
        for ((ref, matches) <- db; e <- matches)
        yield {
          val refN = ref.mRenamed(mkname(ref, "ref.") + _)
          val eN = e.mRenamed(mkname(e, "expt.") + _)
          val lm = refN.bedFile.lmTrack
          val ex = eN.bedFile.exTrack.sync(
            lm       = lm,
            isBefore = Bed.isBefore _,
            canSee   = GenomeLocus.cond(GenomeLocus.Overlap(1)) _,
            isExclusive = false
          ) 
          refN.mOverwritten(eN.meta).trackUpdated {
            for (r <- lm;
                 i = (ex syncedWith r) filter (y => Bed.SameStrand(y, r))) 
            yield r.overwriteMisc(i flatAggregateBy b)
          }
        }
      }
     .Return { DB(_) }
   ...
   */
    ...
    : DB = {
     ...
     .Step1{ db =>
        for((ref, diffs) <- db;
            lm = ref.bedFile.lmTrack;
            ds = for (d <- diffs) 
                 yield d.bedFile.exTrack.sync(
                   lm       = lm,
                   isBefore = Bed.isBefore _,
                   canSee   = GenomeLocus.cond(GenomeLocus.Overlap(1)) _,
                   isExclusive = false); 
            tr = lm.filter { r => ds.forall { d =>
                   d.syncedWith(r).forall { x =>
                     !Bed.SameStrand(r, x) || !overlap(r, x)
                 } } };
            if (tr.hasNext))
        yield ref.trackUpdated(tr)
      }
     .Return { DB(_) } 
    }
