      ...
      region:GroupByRegion[R] = NoRegionGroup)(db:DB) =
    {
      ...
      def mkR[A](r:BObj[A]) = {
        ...
        val g = r.asInstanceOf[BObj[String]] match {
                    ...
                    case _ => Some(r) } 
        (u:Bed) => (u.chrom, u.chromStart, u.chromEnd, u.strand,
                    g match {
          ...
          if (ps.isEmpty) { DB(for(s <- db.samples) yield 
            s updateT {
               (for((k,v)<-s.track().groupby(grp)(keep))
                yield {mkgrp(SimpleBedEntry(k._1,k._2,k._3,k._4), k._5)}).
               iterator})}

          else { DB(for(s <- db.samples) yield 
            s updateT {
                // println("Print DB")
  // for (s <- db.samples) s.track().foreach(println)
               (for((k,v)<-s.track().groupby(grp)(b))
                yield {mkgrp(SimpleBedEntry(k._1,k._2,k._3,k._4, v), k._5)}).  // here is random
               iterator})} } } }.
