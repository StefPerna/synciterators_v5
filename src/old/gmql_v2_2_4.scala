      ...
      def synchroMap(ref: Sample, e: Sample) = {
        ...
        val trs   = SynchroEIterator.map(
                      ...
                      (r: Bed, i: Vector[Bed]) => 
                         r.strand match {
                           case "." =>
                             val t = i.filter(_.strand == ".")
                             val p = i.filter(_.strand != "-")
                             val n = i.filter(_.strand != "+")
                             if (t.length == i.length) 
                               Vector(overwriteMisc(r, i flatAggregateBy b))
                             else
                               Vector(
                                 overwriteMisc(r, p flatAggregateBy b),
                                 overwriteMisc(r, n flatAggregateBy b))
                           case _ =>
                             Vector(overwriteMisc(r, i flatAggregateBy b))
                         }
                     }
        val tr    = for (rs <- trs; r <- rs) yield r
      ...
      def minus(ref: Sample, diffs: Iterator[Sample]) = diffs.hasNext match {
        ...
        case true =>
          ...
          val tr = SynchroEIterator.map(
                     ...
                     grpscreen= (r: Bed, tt: Vector[Bed]) => r.strand match {
                       case "." => tt.filter(_.strand != "+").isEmpty ||
                                   tt.filter(_.strand != "-").isEmpty
                       case _   => tt.isEmpty
                     },
                     n        = 1,
                     convex   = true,
      ...
      def apply(it: Iterator[Bed]) = {
        ...
        for(b <- it;
            if (tmp match {
              ...
              case Some(x) => if ((b sameLocus x) && Bed.SameStrand(b, x)) false
