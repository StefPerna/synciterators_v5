...
object GMQL {
  ...
  object DB 
  {
    ...
    def extend(sample:OnSample[SProj])(db:DB) =
    {
      ...
      if (sproj.isEmpty & sprojAggr.isEmpty) db else DB( for(s <- db.samples) yield  
    ...
    { 
      ...
      Step1{ db=> DB(
        for((ref,matches) <- db; e <- matches) yield {
          ...
          val tr= for(r<-lm) 
                  yield r.mergeMisc((ex syncWith r).flatAggregateBy(b))
      ...
      { //
        ...
        DB(for((anchor, expts) <- db;
             ...
             ex = Connectors.join(
                    lmtrack=lm,
                    extrack=e.track(),
                    canSee=Bed.cond(GenomeLocus.DLE(limit)) _);
             tr = for(r <- lm;
                      t <- ex syncWith r;
                      if (canJoinR(r, t));
                      if (GenomeLocus.cond(pred.g:_*)(t,r));
                      joined <- outputR(r,t))
                  yield {println("joined:"); println(joined); joined.mergeMisc(Map("joinedWith" -> t.name))})
      ...
      Step1{ db =>
        ...
        DB(for((anchor, expts) <- db;
             ...
             tr = for(r <- lm;
                      ...
                      if (GenomeLocus.cond(pred.g:_*)(t,r));
                      joined <- outputR(r,t))
                  yield {println("joined:"); println(joined); joined.mergeMisc(Map("joinedWith"->r.name))})
         yield outputS(anchor, e).updateT(orderR(tr))) }.
  ...
  { //
    ...
    def connect(
      ...
      canSee:(Bed,Bed)=>Boolean = Bed.cond(GenomeLocus.Overlap(1)) _,
