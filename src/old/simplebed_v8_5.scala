...
object BedWrapper {
  ...
  extends LocusLike with Serializable
  {
    ...
    //   s"name=${name}, score=${score}, strand=${strand}, map=${misc} }"
        ...
        override def toString = this.bedFormat
    ...
    def transformMisc(f:Map[String, Any] => Map[String, Any]) =
      SimpleBedEntry(
        chrom, chromStart, chromEnd, name,
        score, strand, f(misc))
  }
  ...
  object BedFile
  {
    ...
    //
    // Serializer and deserializer for Bed files
    //

    def defaultDeserializerBedFile
        (customization: Bed => Bed)
        (filename:String)
    : BedEIterator =
    {
      ...
      def parseBed(ois:(BufferedSource, Iterator[String])):Bed =
      {
        ...
        val e = try ois._2.next()  
                catch {case _ :Throwable=> ois._1.close(); throw FileEnded("")}
    ...
    { //
      // Customization for extracting ENCODE narrow peak 
      // attributes from misc. Encode Narrow Peak files
      // have the standard 6 Bed fields, followed by 4 
      // ENCODE narrow peak-specific fields.
    ...
    def onDiskBedFile(
      ...
      settings: EFileSettings[Bed] = defaultSettingsBedFile) =
    {
      apply(OnDisk[Bed](filename, customization, settings))
    }
    ...
    def onDiskEncodeNPBedFile(
      ...
      settings: EFileSettings[Bed] = defaultSettingsBedFile) =
    {
      onDiskBedFile(filename, toEncodeNPBedFile _, settings)
    }
...
for(x <- bfA.iterator) println(x.getMisc[Double]("peak"))
