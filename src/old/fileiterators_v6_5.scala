...
object FileCollections {
  ...
  case class EFile[A](efile: EFileState[A])
  {
    ...
    //
    // Function to process elements on the EFile
    //
  ...
  object EFile
  {
    ...
    def setDefaultsEFile[A<:Serializable](): EFileSettings[A] =
      EFileSettings[A](
        ...
        samplingSz = 30,
        alwaysOnDisk = false,
        totalsizeOnDisk = filesizeOf[A] _ andThen ((x:Long) => x.toDouble),
        serializer = defaultSerializerEFile[A] _,
        deserializer = defaultDeserializerEFile[A] _) 
    ...
    def totalsizeOnDiskOf[A](efobj:EFile[A]): Long = efobj
                                                       .efile
                                                       .settings
                                                       .totalsizeOnDisk(efobj)
                                                       .toLong
 ...
 */
  ...
  object implicits {
    ...
    implicit def EFile2AggrIter[A](efobj:EFile[A]):AggrIterator[A] = 
      AggrIterator[A](efobj.iterator)
