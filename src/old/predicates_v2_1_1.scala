...
// import synchrony.gmql.GMQL.DB
  ...
  type SampleFile = Samples.SampleFile
  ...
  object SemiJoin {
    ...
    // We actually want the exDB to be of type DB.
    // But we cant define it in terms of DB because
    // DB is defined in GMQL, and GMQL includes this 
    // module as a subobject To break this circularity,
    // we create this SemiJoin companion object with
    // an apply method that accepts a generalisation
    // of DB (viz. { val samples: SampleFile })...

    def apply(
      incl: Incl = Incl(),
      excl: Excl = Excl(), 
      exDB: { val samples: SampleFile }): 
    SemiJoin = {

      import scala.language.reflectiveCalls

      new SemiJoin(incl, excl, exDB.samples.toVector)
