...
object GMQL {
  ...
  object DB 
  {
    ...
    def sortRBy(ordering:Ordering[Bed])(db:DB) = 
      DB(for(s <- db.samples) yield s.sortTBy(ordering))

    def sortRByChromEnd(db:DB) = sortRBy(GenomeLocus.orderByChromEnd)(db)

    def sortRByChromStart(db:DB) = sortRBy(GenomeLocus.orderByChromStart)(db)

    def sortR(db:DB) = sortRBy(GenomeLocus.ordering)(db)
    ...
    { 
      ...
      println(s"Time before Step 0: ${(System.nanoTime - starttime)/1e9} s.")
      Step0{
        mapS(joinby.p:_*)(rdb, edb) }.

      Step1{ 
        println(s"Time at Step 1: ${(System.nanoTime - starttime)/1e9} s.")
        db=> DB(for((ref,matches) <- db; e <- matches) yield {
        ...
        val a = refN.overwriteM(eN.meta).updateT(tr) 
    println(s"Time at end of Step 1: ${(System.nanoTime - starttime)/1e9} s.")
    a} )
      }. 
