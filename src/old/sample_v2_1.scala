...
object Samples {
  ...
  case class Sample(meta:Meta, track:()=>Iterator[Bed])
  {
    ...
    def sortT() = sortTBy(GenomeLocus.ordering)

    def sortTByChromEnd() = sortTBy(GenomeLocus.orderByChromEnd)

    def sortTByChromStart() = sortTBy(GenomeLocus.orderByChromStart)

    def sortTBy(ordering:Ordering[Bed]) =
    {
      val t = track().toSeq.sorted(ordering)
      Sample(meta, ()=> t.iterator)
    }


    def sortTWith(cmp:(Bed,Bed)=>Boolean) =
    {
      val t = track().toSeq.sortWith(cmp)
      Sample(meta, ()=> t.iterator)
    }


    def updateT(it:Iterator[Bed]) =
    { val time = System.nanoTime(); val t = it.toVector; println(s"I did the list in ${(System.nanoTime() - time)/1e9}"); Sample(meta, () => t.iterator) } // this seems costly
    /*
