 ...
 *        where transparently materializes the parts of the
...
object SampleFileOps {
  ...
  val SampleFile = Samples.SampleFile
  ...
  type SampleFile      = Samples.SampleFile      // Same as EFile[Sample]
  type SampleEIterator = Samples.SampleEIterator // Same as EIterator[Sample]
  type Bed             = BedFileOps.Bed
  type BedFile         = BedFileOps.BedFile      // Same as EFile[Bed]
  type BedEIterator    = BedFileOps.BedEIterator // Same as EIterator[Bed]
 ...
 */
    ...
    def tracksSortedIfNeeded: SampleFile = tracksAreSorted match {
      case true  => samples
      case false => samples.tracksSorted
    }
 ...
 */
    ...
    def tracksAreSorted: Boolean = samples.forall(_.bedFile.isSorted)
 ...
 *  WHERE joinby(s, t) && (
 *        exact && locus of s is not any tracks in t;  OR
 *        !exact && locus of s does not overlap any region in t.)
 */
 ...
 *  {
       import BFOps.coverR, BFCover.{ coverGen, between }
       db.coverS(
 ...
 *  }
 *
 *  Note that BFOps.coverR also produces the counts (viz. accIndex)
 *  for histograms. So we get GMQL's HISTOGRAM for free above.
 */

    def coverS(
      onRegion: Seq[BedFile] => BedFile,
      onSample: String*
    ): SampleFile = SFOps.coverS(samples)(onRegion, onSample: _*)
 ...
 */
  ...
  object SFOps {
    ...
    def coverS
      ...
      (onRegion: Seq[BedFile] => BedFile,
       onSample: String*)
    : SampleFile = onSample.length match {
      ...
      case _ => SampleFile.transientSampleFile {
        import synchrony.iterators.AggrCollections.implicits._
        val l = (s: Sample) => Map(onSample.map(g => g -> s[Any](g)): _*)
        val p = for (
