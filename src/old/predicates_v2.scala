...
object Predicates {
  ...
  case class SemiJoin(incl:Incl=Incl(), excl:Excl=Excl(), exDB:DB) 
  extends SPred {
    def apply(s:Sample) = exDB.samples.iterator.exists(t =>
      incl.f.forall (l=>s.checkM[Any](l,u=>t.checkM[Any](l,v=>u==v))) &&
      excl.f.forall (l=>s.checkM[Any](l,u=>t.checkM[Any](l,v=>u!=v,true),true)))
