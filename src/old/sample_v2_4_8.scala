...
object Samples {
   ...
   */
    ...
    def trackSavedAs(
      name: String,
      preparation: Option[Bed => Bed] = None)
    : Sample = Sample.saveTrackAs(this)(name, preparation)
   ...
   */
    ...
    def saveTrackAs
      (s: Sample)
      (name: String, preparation: Option[Bed => Bed] = None): Sample = {
      
      new Sample(s.meta, s.bedFile.savedAs(name, preparation))
   ...
   */
    ...
    : SampleFile = {
      ...
      SampleFile.transientSampleFile(ns).savedAs(importAs)
    }
