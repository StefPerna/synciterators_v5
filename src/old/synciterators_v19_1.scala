...
// assumptions are the monotonocity conditions (i) and (ii), and
// the antimonotonicity conditions (iii) - (iv) below. Let x and y
...
// the monotonicity and antimonotonicty conditions sufficient to
...
//
// (iii) If x << x', then for all y in Y: y isBefore x, 
// and not y canSee x, implies not y canSee x'.
//
// (iv) If y << y', then for all x in X: not y isBefore x,
// and not y canSee x, implies not y' canSee x.
//
...
object SyncCollections {
  ...
  { //
    ...
    def apply[B](it: Vector[B]): LmTrack[B] = SynchronyIterator[B](it)
  }
  ...
  trait Synchrony[B]
  {
    ...
    { //
      ...
      { //
        ...
        if (exclusiveCanSee) { 
          itA drop (toSync.length)
          //
          // if exclusiveCanSee, discards from itA every element
          // that got shifted just now.
          //
        } else {
          itA filterBuffer (a => !isBefore(a,b) || canSee(a,b))
          //
          // Remove elems in itA buffer that cannot see the next
          // elem of itB, in accordance to antimonotonicity assumption.
        } 
  ...
  object SynchronyIterator
  {
    ...
    def apply[B](it:Vector[B]):Iterator[B] with Synchrony[B] =
      SynchronyIterator(it.iterator)
  }
