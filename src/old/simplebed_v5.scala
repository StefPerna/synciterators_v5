 ...
 */
    ...
    def orderByChromEnd[A<:Bed]:Ordering[A] =
      Ordering.by(l => (l.chrom, l.strand, l.chromEnd, l.chromStart))

    def orderByChromStart[A<:Bed]:Ordering[A] =
      Ordering.by(l => (l.chrom, l.strand, l.chromStart, l.chromEnd))
    ...
    def cond(ps:GenomeLocus.LocusPred*)(x:Bed, y:Bed) = 
      (x.chrom==y.chrom) && (x.strand==y.strand) && ps.forall(p => p(x,y))
