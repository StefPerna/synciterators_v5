...
object Samples {
  ...
  object SampleFile
  {
    ...
    val settingsSampleFile: EFileSettings[Sample] = EFileSettings[Sample](
      prefix = "synchrony-",
      suffixtmp = ".sftmp",
      suffixsav = ".sfsav",
      aveSz = 1000,
      ...
      samplingSz = 100,
      alwaysOnDisk = false,
      serializer = serializeSampleFile _,
      deserializer = deserializeSampleFile _) 


    // Alternate settings, to use alternate deserializer
    // for reading SampleFile generated by Stefano Perna.

    def altSettingsSampleFile(dbpath:String): EFileSettings[Sample] =
    {
      EFileSettings[Sample](
        ...
        alwaysOnDisk = false,
        serializer = serializeSampleFile _,
        deserializer = altDeserializeSampleFile(dbpath)) 
    }



    // altDeserializeIt is the deserializer for reading
    ...
    def altDeserializeSampleFile(dbpath:String) = (samplelist:String) =>
    {
      ...
      def parseSample(dbPath:String) = (ois:Iterator[String]) =>
      {
        val sid = ois.next().trim
        val mf = s"${dbPath}/${sid}.meta"
        val bf = s"${dbPath}/${sid}"
        ...
        try { Sample(Map(fields.toSeq :_*) + ("sid" -> sid), 
                     onDiskBedFile(bf)) }
        finally { metafile.close() }
      }

 
      if ((samplelist endsWith settingsSampleFile.suffixtmp) ||
          (samplelist endsWith settingsSampleFile.suffixsav))
      { //
        // Hey, the file is in Limsoon's format,
        // use Limsoon's deserializer.
      
        deserializeSampleFile(samplelist)
      }
      else
      { //
    ...
    // deserializeIt is deserializer for Limsoon's prefered
    // format for representing samples. All the samples are
    // in one file, separated by single blank lines. 
    // Each meta attribute is on separate line, and is has
    // the form fieldname \t fieldvalue.  Moreover, the first
    // line is always the filename of the associated Bed file.


    def deserializeSampleFile(samplelist: String): SampleEIterator =
    {
      def openFile(f:String) =
      {
      ...
      def parseSample(ois:Iterator[String]) =
      {
        val sid = ois.next()
      ...
      EIterator.makeParser(openFile, parseSample, closeFile)(samplelist) 
    }


    def serializeSampleFile(filename:String, it:SampleEIterator) =
    {
    ...
    def inMemorySampleFile(
      ...
      settings: EFileSettings[Sample] = settingsSampleFile) =
    {
      apply(InMemory(entries, settingsSampleFile))
    }
    ...
    def transientSampleFile(
      ...
      settings: EFileSettings[Sample] = settingsSampleFile) =
    {
      apply(Transient(entries, settingsSampleFile))
    }
    ...
    def onDiskSampleFile(
      ...
      settings: EFileSettings[Sample] = settingsSampleFile) =
    {
      apply(OnDisk(filename, settingsSampleFile))
    }


    // For reading the files created by Stefano Perna.

    def altOnDiskSampleFile(dbpath: String, samplelist:String): SampleFile =
    {
      apply(OnDisk(samplelist, altSettingsSampleFile(dbpath)))
    }
  }
}
...
/*
 *
 * Here is an example on using SampleFile...
 *
 *
