...
object GenomeAnnot {
  ...
  object GenomeLocus 
  {
    ...
    implicit def ordering[A<:LocusLike]: Ordering[A] = orderByChromEnd
    //
    // It is possible to order locus based on chromEnd first,
    // or based on chromStart first. I choose chromEnd first
    // here. You can change this to chromStart first. But
    ...
    (x.chrom < y.chrom) || (cond(StartBefore)(x,y))
