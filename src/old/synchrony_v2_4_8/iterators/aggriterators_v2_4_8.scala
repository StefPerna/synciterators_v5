...
object AggrCollections {
   ...
   */
    ...
    def average[C](f: C => Double): Sri[C, Double] = 
      combine[C, Double, Int, Double](sum(f), count){
   ...
   */
    ...
    def count[C]: Sri[C, Int] = sri[C, Int, Int](
      () => 0,
      x  => 1,
      (x, y) => x + y
    )
   ...
   */
    ...
    def stats[C](f: C => Double): Sri[C, Stats] = {
      ...
      def mix(es: Array[AnyVal]) =
      {
        val cnt = es(0).asInstanceOf[Int]
        val sum = es(1).asInstanceOf[Double]
        val ssq = es(2).asInstanceOf[Double]
        val min = es(3).asInstanceOf[Double]
        val max = es(4).asInstanceOf[Double]
