 ...
 */
    ...
    : Seq[BedFile] => BedFile = (vs: Seq[BedFile]) => vs.length match {
      ...
      case _ =>
        ...
        val ends   = mm(merged.eiterator.map(_.end))
        val rf     = BFOrder.distinct(starts.mergedWith(ends).eiterator).stored
        starts.destruct()
        ends.destruct()
        ...
        val tr = SynchroEIterator.ext(
          isBefore = Bed.isBefore _,
          canSee   = overlap,
          screen   = Bed.cond(Locus.EndBefore or Locus.EndAfter) _,
          convex   = false,
          early    = false,
 ...
 */
  ...
  object BFOrder {
    ...
    def histo(minmax: Int => Boolean) = { (it: Iterator[Bed]) =>
      ...
      val tr  = for (
                  cur <- eit;
                  if minmax(cur[Int]("accIndex"))
                ) yield eit.peekahead(1) match {
                   case None => cur
                   case Some(nxt) => cur.overlap(0)(nxt) match {
                     case true  => Bed(cur.chrom, cur.chromStart, 
                                       nxt.chromStart, cur.strand, 
                                       cur.name, cur.score, cur.misc)
                     case false => cur
                   }
                }
      BedFile.transientBedFile(tr)
    ...
    def summit(minmax: Int => Boolean) = { (it: Iterator[Bed]) =>
      ...
      def isSummit(cur: Bed) = eit.peekahead(1) match {
        case None      => { up = true; true }
        case Some(nxt) => cur.overlap(0)(nxt) match {
          case false => { up = true; true }
          case true  => 
            (cur[Int]("accIndex") > nxt[Int]("accIndex")) match {
               case false => { up = true; false }
               case true  => { up = false; true }
          }
        }
      }
    ...
    def flat(minmax: Int => Boolean) = { (it: Iterator[Bed]) =>
      val tr = new EIterator[Bed] {
        val eit = EIterator[Bed](it).filter(b => minmax(b[Int]("accIndex")))
        override def myClose(): Unit = eit.close()
        override def myHasNext: Boolean = eit.hasNext
        override def myNext(): Bed = {
          var acc = eit.next()
  ...
  object BFCover {
    ...
    def coverGenMin = (r: Bed, hs: Vector[Bed]) => hs.isEmpty match {
      ...
      case false => 
        ...
        Vector(Bed(ch, cs, ce, sr, "", 0, Map("accIndex" -> cn)))
    }
