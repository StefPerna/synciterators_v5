      ...
      if (sample.p.isEmpty && region.p.isEmpty) db 
        ...
        yield  
          if (region.p.isEmpty) s 
            ...
            for(r <- s.track; if region.p.forall(f => f(s,r))) yield r,
            s.trackSz) ) 
    }
    ...
    { 
      ...
      Step1{ db=> DB(
        for((ref,matches) <- db; e <- matches) yield {
          ...
          refN.overwriteM(eN.meta).updateT(tr, ref.trackSz + eN.trackSz) } ) }. 
      ...
      Step1{ db => DB( 
        for((ref, diffs) <- db;
            ...
            tr = for(r <- lm; 
                 ...
                 yield r;
            if (tr.hasNext))
        yield ref updateT (tr, ref.trackSz)) }. 
        ...
        DB(for((anchor, expts) <- db;
             ...
             tr = for(r <- lm;
                   ...
                   // if (Bed.cond(pred.g:_*)(t,r));
                      ...
                      joined <- outputR(r,t))
                  yield joined)
         yield outputS(anchor, e).updateT(orderR(tr), anchor.trackSz + e.trackSz))
      } }.
