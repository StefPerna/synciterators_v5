...
object AggrCollections {
  ...
  { //
    ...
    final def groupby[B>:C,K,F](f:B=>K)(aggr: Sri[B,F]): Map[K,F] =
    { //
    ...
    final def partitionby[B>:C,K,F](f:B=>K)(aggr: Sri[B,F]): Iterator[(K,F)] =
    { //
    ...
    final def aggregateBy[B>:C,F](aggr: Sri[B,F]): Vector[F] =
    { //
    ...
    final def flatAggregateBy[B>:C,F](aggr: Sri[B,F]):F =
    {
