...
object GMQL {
  ...
  case class DB(samples:SampleFile)
  {
    ...
    def savedAs(file:String) = DB.saveAs(this, file)
  ...
  object DB 
  {
    ...
    { //
      ...
      new DB(onDiskSampleFile(samplelist, customization, settings))
    }


    def altOnDiskDB(dbpath:String)
        (samplelist:String,
         customization:Sample => Sample = nullCustomizationSampleFile _,
    ...
    { //
      ...
      new DB(altOnDiskSampleFile(dbpath)(samplelist, customization, settings))
    }


    def altOnDiskEncodeNPDB(dbpath:String)
        (samplelist:String,
         settings: EFileSettings[Sample] = altSettingsSampleFile(dbpath))
    : DB =
    { //
      ...
      altOnDiskDB(dbpath)(samplelist, toEncodeNPSampleFile _, settings)
    }
    ...
    def saveAs(db:DB, file:String):DB = DB(db.samples.savedAs(file))
    ...
    def extend(sample:OnSample[SProj])(db:DB) =
    {
      ...
      Step0{
        ...
        while (ps.hasNext) { 
          ...
          snew = snew.overwriteM(ps.next()(s))
        }
        snew }.
      ...
      Step1{ snew =>
        ...
        { //
          ...
          snew overwriteM (s.track.flatAggregateBy(aggr))
        } }.
      ...
      Step2{ snew => snew.updateT (
        ...
        { 
          ...
          while (ps.hasNext) {
            ...
            bed = bed overwriteMisc (ps.next()(s,r))
          }
          bed 
        }) }.
        ...
        { 
          ...
          if (x.aggr.isEmpty) DB {
            (for ((g,acc)<-db.samples.iterator.groupby(x.grp(_))(keep); s<-acc)
             yield s overwriteM (Map("group" -> g))).
            toVector }
          else DB {
            (for ((g,(c,acc))<-db.samples.iterator.groupby(x.grp(_))(withAcc(b));
                  s<-acc)
             yield s overwriteM (Map("group" -> g) ++ c)).
            toVector } } } }.
