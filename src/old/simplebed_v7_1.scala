...
object BedWrapper {
  ...
  //
  // BedEIterator(file) produces an EIterator
  // which iterates on the file. The iterator
  // comes with a close() function to close
  // the file mid-way during iteration.
  //
  ...
  object BedEIterator {
    ...
    def apply(file:String):BedEIterator = 
      EIterator.makeParser(openFile, parseBed, closeFile)(file)


    import scala.io.{Source, BufferedSource}


    private def openFile(f:String) =
    {
    ...
    private def closeFile(ois:(BufferedSource, Iterator[String])) =
      ois._1.close()


    private def parseBed(ois:(BufferedSource, Iterator[String])):Bed =
    {
      val e = try { ois._2.next() } 
              catch { case _ :Throwable => throw FileEnded("") }
              finally { ois._1.close() }
      ...
      SimpleBedEntry(chrom, chromStart, chromEnd, name, score, strand, Map())
    }
  }


  //
  // BedFile(file) produces an EFile[Bed].
  // The EFile let us iterate repeatedly on the file.
  // It also let us serialize the iterator,
  // merge it with another EFile[Bed], and
  // sort the file externally.
  //
  ...
  object BedFile {
    ...
    def apply(file:String):BedFile = UserEFile(file, BedEIterator.apply)

    def apply(
      entries:Iterator[Bed],
      sz:Long = EFile.szThreshold,
      onDisk:Boolean = onDisk):BedFile = EFile(entries, sz, onDisk)

    def fromVector(
      entries:Vector[Bed],
      onDisk:Boolean = onDisk):BedFile = EFile.fromVector(entries, onDisk)
  }
...
val bfA = BedFile(fileA)
val bfB = BedFile(fileB)
