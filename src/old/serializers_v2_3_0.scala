...
object Serializers {
  ...
  trait Formatter[A] {
    def formatWithHeader(a: A): String = ""
  ...
  class ItemFormatter[A] extends Formatter[A] {
    override def formatWithHeader(a: A): String = ItemFormatter.format[A](a)
    ...
    EIterator[A] = new EIterator[A] {
      ...
      private val ois = slurp match {
        ...
        case None =>  
          ...
          itemParser.parseHeader(EIterator.StringfromFile(filename, guard))

        // Assume "filename" is a slurped copy of the file f.
        case Some(f) => 
          itemParser.parseHeader(EIterator.fromVector(filename.split("\n")))
      }
