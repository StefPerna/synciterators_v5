 ...
 */
  ...
  /**
   * Below is a version using synchrony iterator in comprehension syntax.
   * It is just as fast as the synchroMap version above,
   */ 

     DB {
        for (ref <- rdb.samples;
             refN = ref.mRenamed(mkname(ref, "ref.") + _);
             e   <- edb.samples;
             if joinby.p.forall(_(ref, e));
             eN = e.mRenamed(mkname(e, "expt.") + _);
             lm = ref.track;
             ex = e.track;
             tr = for (r <- lm;
                       i = ex.syncedWith(r)(
                                isBefore = Bed.isBefore _,
                                canSee   = GenomeLocus.cond(
                                             GenomeLocus.Overlap(1)) _,
                                screen   = Bed.cond() _))
                  yield overwriteMisc(r, i flatAggregateBy b);
             bed   = BedFile.transientBedFile(tr).serialized)
   ...
   */
    ...
    DB = {
      ...
      def synchroJoin(anchor: Sample, e: Sample) = {
        ...
        val synchro = SynchroEIterator(
                        isBefore = Bed.isBefore _,
                        canSee   = GenomeLocus.cond(ycs: _*) _,
                        screen   = screen _,
                        extrack  = e.track,
                        lmtrack  = anchor.track)
        val tr      = for ((r, tt) <- synchro; 
                                 t <- tt;
                           joined <- outputR(r, t))
                      yield joined
        val otr     = orderR match {
                        case DoneR         => tr
                        case SortR         => tr
  ...
  // //
  // // The SemiJoin defined in synchrony.gmql.Predicates
  // // requires exDB to be Vector[Sample]. We put here a
  // // version of SemiJoin predicate that takes a DB as exDB.
  // //

  // case class SemiJoin(
  //   incl: Incl = Predicates.Incl(),
  //   excl: Excl = Predicates.Excl(),
  //   exDB: DB)
  // extends Predicates.SPred{

  //   def apply(s: Sample) = Predicates.SemiJoin(incl, excl, exDB.toVector)(s)
  ...
  object implicits {
    ...
    implicit def DB2Vector(db: DB): Vector[Sample] = db.toVector
