...
object BedWrapper {
  ...
  //
  // Use "+" for strand when strand value is neither "+" nor "-"
  //
  val defaultPlus = true
  def validateStrand(s:String) = if (defaultPlus && s != "-") "+" else s
  ...
  extends LocusLike
  {
    ...
    override def toString = // simpler .toString for testing purposes
...
/*
 *
    def apply(chrom:String, chromStart:Int, chromEnd:Int) =
    {
      new SimpleBedEntry(
 ...
 */
    ...
    def orderByChromEnd[A<:Bed]:Ordering[A] =
      Ordering.by(l => (l.chrom, validateStrand(l.strand),
                        l.chromEnd, l.chromStart))

    // def orderByChromStart[A<:Bed]:Ordering[A] =
    //   Ordering.by(l => (l.chrom, validateStrand(l.strand),
    //                     l.chromStart, l.chromEnd))

    def orderByChromStart[A<:Bed]:Ordering[A] =
      Ordering.by(l => (l.chrom, 
                        l.chromStart, l.chromEnd,
                        validateStrand(l.strand)))
    ...
    { //
      ...
      (x.chrom==y.chrom) && 
      //(validateStrand(x.strand) == validateStrand(y.strand)) && 
      ps.forall(p => p(x,y))
    }
    ...
    case object SameStrand extends GenomeLocus.LocusPred {
      def apply(x:LocusLike, y:LocusLike) = (x,y) match
        {case (u:Bed,v:Bed)=>validateStrand(u.strand)==validateStrand(v.strand)
         case _ => true }
    }
