...
object Samples {
  ...
  case class Sample(meta:Meta, bedFile:BedFile) extends Serializable
  {
    ...
    def checkM[T](f:String, chk:T=>Boolean, whenNotFound:Boolean = false) =
      meta.get(f) match { 
    ...
    def updateBedFile(it:BedFile) = new Sample(meta, it)
  ...
  object SampleFile
  {
    ...
    def totalsizeOnDiskSampleFile(efobj:SampleFile):Double = efobj.efile match
    {
