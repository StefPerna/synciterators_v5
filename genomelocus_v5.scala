...
object GenomeAnnot {
  ...
  object GenomeLocus 
  {
    ...
    def isBefore(x:LocusLike, y:LocusLike) =
      (x.chrom < y.chrom) || (cond(EndBefore)(x,y))
    ...
    trait LocusPred
    { //
      // Various "topological predicates". Note that these predicates
      // assume they are testing two loci on the same chromosome. 

      def apply(x:LocusLike, y:LocusLike) = true
    }


    case class And(u:LocusPred, v:LocusPred) extends LocusPred
    {
      override def apply(x:LocusLike, y:LocusLike) = u(x,y) && v(x,y)
    }
       
    case class Or(u:LocusPred, v:LocusPred) extends LocusPred
    {
      override def apply(x:LocusLike, y:LocusLike) = u(x,y) || v(x,y)
    }
    ...
    {
      override def apply(x: LocusLike, y:LocusLike) =
        x.chromStart < y.chromStart
    }

    case object EndBefore extends LocusPred
    {
      override def apply(x: LocusLike, y:LocusLike) = x.chromEnd < y.chromStart
    }
   
    case object StartAfter extends LocusPred
    {
      override def apply(x:LocusLike, y:LocusLike) = x.chromStart > y.chromEnd
    }

    case object EndAfter extends LocusPred
    {
      override def apply(x:LocusLike, y:LocusLike) = x.chromEnd > y.chromEnd
    }
    ...
    case object Inside extends LocusPred  
    {
      override def apply(x:LocusLike, y:LocusLike) =
        (x.chromStart >= y.chromStart) &&
        (y.chromEnd >= x.chromEnd)
    }

    case object Enclose extends LocusPred 
    {
      override def apply(x:LocusLike, y:LocusLike) = Inside(y, x)
    }

    case object Outside extends LocusPred 
    {
      override def apply(x:LocusLike, y:LocusLike) = 
        !Inside(x,y) && !Enclose(x,y)
    }
    ...
    {
      override def apply(x: LocusLike, y:LocusLike) = dist(x,y) < n
    }
